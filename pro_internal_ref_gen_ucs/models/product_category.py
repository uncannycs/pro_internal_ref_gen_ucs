...
class ProductCategory(models.Model):
    ...
    def action_generate_category_references(self):
        """Action button to trigger reference generation for all products in this category and subcategories."""
        self.ensure_one()
        templates = self.env['product.template'].search([('categ_id', 'child_of', self.id)])
        if not templates:
